...
fabric.BaseBrush.prototype.convertToImg = function () {
  ...
  img
    .set({
      left: xy.x / (pixelRatio * zoom),
      top: xy.y / (pixelRatio * zoom),
      scaleX: 1 / (pixelRatio * zoom),
      scaleY: 1 / (pixelRatio * zoom),
    })
    .setCoords();
 ...
 */
export const CrayonBrush = fabric.util.createClass(fabric.BaseBrush, {
  color: "#000",
  opacity: 0.6,
  width: 30,
 ...
 * Based on code by Tennison Chan.
 */
export const SpraypaintBrush = fabric.util.createClass(fabric.BaseBrush, {
  color: "#000",
  opacity: 1,
  width: 30,
  ...
  onMouseDown: function (pointer) {
    this.canvas.contextTop.globalAlpha = this.opacity;
    this._point = new fabric.Point(pointer.x * 4, pointer.y * 4);
 ...
 * Based on code by Tennison Chan.
 */
export const MarkerBrush = fabric.util.createClass(fabric.BaseBrush, {
  color: "#000",
  opacity: 1,
  width: 30,
  ...
  _render: function (pointer) {
    ...
    for (i = 0, len = this._size / this._lineWidth / 2; i < len; i++) {
      ...
      ctx.moveTo(
        this._lastPoint.x * this._zoom + lineWidthDiff,
        this._lastPoint.y * this._zoom + lineWidthDiff
      );
      ctx.lineTo(
        pointer.x * this._zoom + lineWidthDiff,
        pointer.y * this._zoom + lineWidthDiff
      );
      ctx.stroke();
    }
 ...
 */
export const RibbonBrush = fabric.util.createClass(fabric.BaseBrush, {
  color: "#000",
  opacity: 1,
  width: 1,
  ...
  update: function () {
    ...
    for (var i = 0; i < painters.length; i++) {
      ctx.beginPath();
      ctx.moveTo(painters[i].dx * this._zoom, painters[i].dy * this._zoom);
      ...
      painters[i].dy -= painters[i].ay =
        ...
        painters[i].ease;
      ctx.lineTo(painters[i].dx * this._zoom, painters[i].dy * this._zoom);
      ctx.stroke();
    }
  },
  ...
  onMouseDown: function (pointer) {
    ...
    for (var i = 0; i < this._nrPainters; i++) {
      this._painters[i].dx = pointer.x * 1;
      this._painters[i].dy = pointer.y * 1;
    }
